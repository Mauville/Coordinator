...
def saucer(query, WorkingIndex, Index, Wait, outputfolder, imagefolder):
...
diditexist("~\\Documents\\Coordinator\\Input")
diditexist("~\\Documents\\Coordinator\\Output")
if diditexist("~\\Documents\\Coordinator\\Output\\Images") == False:
    print("Se han creado directorios en la carpeta de documentos. Ingresa los archivos necesarios en Input.\n"
          "Para mas informacion consulta Ayuda.")
    time.sleep(7)
    sys.exit()
...
WorkingIndex = 0
mistakenumber = 0
connection = str(input("Tu conexion a internet es Buena (1) Regular (2) Mala (3)\n"))
...
while WorkingIndex <= LastIndex:
    ...
    SAUCE = saucer(query, WorkingIndex, Index, wait, OutputFolder, ImageFolder)
